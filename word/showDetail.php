...
if (isset($_SESSION['ngMsg'])) {
    $ngMsg = $_SESSION['ngMsg'];

    $smarty->assign('ngMsg', $ngMsg);
    unset($_SESSION['ngMsg']);
}
...
try {
    ...
    if (!wordCheck($_GET['comment'])) {
        $_SESSION['ngMsg'] = 'コメントにNGワードが含まれています。';
        header('Location:/wordy/word/showDetail.php?wordId='.$wordId.'&word='.$word);
    }else if(isset($_GET['isAdd']) && $_GET['comment'] != ''){
